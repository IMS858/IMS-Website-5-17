...
(function () {
  ...
  const STYLES = `
    ...
    .ims-cb-fallback {
      ...
      background: #FFFFFF; border: 1px solid #E3DCD0; border-left: 3px solid #2A85BE; border-radius: 8px;
    }
    .ims-cb-fallback-title { font-family: 'Fraunces', serif; font-weight: 500; font-size: 0.98rem; color: #2B2723; margin: 0 0 4px; }
    .ims-cb-fallback-sub { font-size: 0.82rem; color: #5A524C; margin: 0 0 10px; line-height: 1.4; }
    .ims-cb-fallback-actions { display: flex; flex-direction: column; gap: 6px; }
    .ims-cb-fallback-btn {
      display: block; text-align: center; padding: 9px 12px;
      background: #FAF7F2; color: #174E70; border: 1px solid #E3DCD0; border-radius: 6px;
      font-size: 0.84rem; font-weight: 600; text-decoration: none;
      transition: background 0.15s;
    }
    .ims-cb-fallback-btn:hover { background: #F0EAE0; }
    .ims-cb-fallback-btn--primary {
      background: #2A85BE; color: white; border-color: #2A85BE;
    }
    .ims-cb-fallback-btn--primary:hover { background: #2374A6; }
  `;
  ...
  // Rich fallback card shown when the bot can't connect — gives the user actionable paths forward
  function showFallback() {
    const el = document.createElement('div');
    el.className = 'ims-cb-fallback';
    el.innerHTML =
      "<p class='ims-cb-fallback-title'>I'm having trouble connecting right now.</p>" +
      "<p class='ims-cb-fallback-sub'>You can still reach IMS directly — Jason gets back fast.</p>" +
      "<div class='ims-cb-fallback-actions'>" +
        "<a href='tel:+16199371434' class='ims-cb-fallback-btn ims-cb-fallback-btn--primary'>Call (619) 937-1434</a>" +
        "<a href='/book.html' class='ims-cb-fallback-btn'>Book Free Movement Assessment</a>" +
        "<a href='/contact.html' class='ims-cb-fallback-btn'>Send a Message</a>" +
      "</div>";
    messagesEl.appendChild(el);
    scrollToBottom();
  }
  ...
  async function sendMessage(text) {
    ...
    try {
      ...
      // Always parse the body — server returns structured error codes even on failure
      let data = null;
      try { data = await res.json(); } catch (e) { /* non-JSON response */ }

      if (!res.ok) {
        // Dev diagnostics — open browser console to see exact failure
        console.error('[IMS chatbot]', {
          endpoint,
          status: res.status,
      ...
      const reply = (data && data.reply) || "Sorry, I didn't catch that.";
      addBotMessage(reply);
      ...
      hideTyping();
      console.error('[IMS chatbot] Network/fetch exception:', e);
      showFallback();
    } finally {
      pending = false;
    }
